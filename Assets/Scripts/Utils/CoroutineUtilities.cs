...
{
    public static IEnumerator MoveObjectWithTime(Transform target, Vector3 initial_pos, Vector3 dest_pos, float duration_sec)
    {
    ...
    public static IEnumerator MoveObjectWithSpeed(Transform target, Vector3 initial_pos, Vector3 dest_pos, float speed)
    {
        float initial_time = Time.time;
        float distance = (initial_pos - dest_pos).magnitude;
        // The "progress" variable will go from 0.0f -> 1.0f over the course of "duration_sec" seconds.
        float progress = (Time.time - initial_time) * speed / distance;

        while (progress < 1.0f)
        {
            // Recalculate the progress variable every frame. Use it to determine
            // new position on line from "initial_pos" to "dest_pos"
            progress = (Time.time - initial_time) * speed / distance;
            Vector3 new_position = Vector3.Lerp(initial_pos, dest_pos, progress);
            target.position = new_position;
